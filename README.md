...
The same entry point has three carriers, all generated by the console's
**Add node** flow so nothing is assembled by hand (control repo,
`docs/SYSTEM_SPEC.md` §4):
...
| cloud-init | a VPS | paste the generated user-data into the provider; it runs the same line on first boot |
| USB stick | a mini PC with no monitor | `alwayswork/provision.toml` on a FAT partition; the first-boot timer (`aw provision`) finds it and enrolls |

An unattended installer image with the agent baked in (for a truly blank
mini PC) is designed but not built yet; see the spec.
