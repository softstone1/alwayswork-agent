...
sudo cp inventory/lab-01.yaml /etc/anakut-worker/worker.yaml
sudo aw apply
sudo aw doctor
```
...
`install.sh --from /opt/anakut-worker && aw init --profile P && aw bootstrap`.
...
sudo install.sh --from /opt/anakut-worker     # or re-run the curl installer
...
- Full node: restore `/etc/anakut-worker` and `/var/lib/anakut-worker` from
  `backup.restic`.
